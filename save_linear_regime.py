...
def main():
    ...
    # --- MODIFIED: Removed out_graph, kept out_match ---
    parser.add_argument('--out_match', type=str, default="matching.txt", help='Output file for the final matching (root).')
    args = parser.parse_args()
    N_NODES = args.n
    M_EDGES_PER_NODE = args.m

    # --- All processes track their own data ---
    ...
    if rank == 0:
        ...
        # --- MODIFIED: Removed the "save graph" block ---
    ...
    while True:
        ...
        alive_nodes = comm.bcast(alive_nodes, root=0) 
        ...
        vertex_map = round_params['vertex_map'] 
        hash_seed = round_params['hash_seed']   
        ...
        for u, v in my_subgraph_edges:
            ...
            priority = hash((hash_seed, edge_tuple))
    ...
    if rank == 0:
        ...
        # --- ADDED THIS BLOCK ---
        print(f"[Master] Saving final matching to {args.out_match}...")
        with open(args.out_match, "w") as f:
            f.write(f"# Final maximal matching with {len(Total_Matching)} edges\n")
            # We use the original node IDs from the graph generation
            # Note: This assumes G_current.nodes() are the original IDs
            # This is correct because we never compacted them.
            for u, v in Total_Matching:
                f.write(f"{u} {v}\n")
        # --- END ADD ---
    ...
    if rank == 0:
        ...
        for machine_id in range(size):
            if all_workload_data[machine_id] is None: continue 
